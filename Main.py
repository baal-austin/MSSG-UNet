...
for Neighbors in [0]: #0, 5,10,15,20
    ...
    # for (FLAG, curr_train_ratio,Unet_Depth) in [(1,0.05,4)]: #(1,0.05,4),,(3,0.005,4)(2,0.005,4),(3,0.005,4)(2,0.005,4),(3,0.005,4)
        ...
        if FLAG == 2:
            ...
            gt_mat = sio.loadmat('HSI_data\\PaviaU_gt.mat')
            gt = gt_mat['paviaU_gt']
        ...
        if FLAG == 3:
            ...
            pass
        if FLAG == 4:
            data_mat = sio.loadmat('HSI_data\\KSC.mat')
            data = data_mat['KSC']
            gt_mat = sio.loadmat('HSI_data\\KSC_gt.mat')
            gt = gt_mat['KSC_gt']
        ...
        for curr_seed in Seed_List:
            ...
            # tic = time.clock()
            tic = time.perf_counter()  # 或者 time.process_time()
            ...
            # toc = time.clock()
            toc = time.perf_counter()  # 或者 time.process_time()
