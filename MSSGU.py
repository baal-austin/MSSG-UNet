...
class HiGCN(nn.Module):
    def __init__(self, height: int, width: int, changel: int, class_count: int,
        ...
        # ==========更改===================================
        # 引入多头注意力机制
        # 在图卷积层之间加入多头注意力机制，以增强特征的表达能力。这可以帮助模型更好地捕捉不同节点之间的关系。
        self.attention = MultiheadAttention(embed_dim=192, num_heads=4)
        # ==========更改===================================
    ...
    def forward(self, x: torch.Tensor, showFlag=False):
        ...
        # ==========更改===================================
        # 多头注意力机制处理
        attn_output, _ = self.attention(H_i.unsqueeze(0), H_i.unsqueeze(0), H_i.unsqueeze(0))
        H_i = attn_output.squeeze(0)
        # ==========更改===================================
